...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate Frequency-Aware Complex U-Net with hard k-space DC."
    )
...
def main() -> None:
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            for (
                ...
                fa_unet_mag, _ = target_scaled_magnitude_pair(
                ...
                fa_unet_dc_mag, _ = complex_image_target_scaled_magnitude_pair(
                ...
                if not example_saved:
                    save_image_grid(
                        images=[
                            ...
                            fa_unet_mag,
                            fa_unet_dc_mag,
                            np.abs(fa_unet_dc_mag - target_mag),
                        ],
                        titles=[
                            "Ground Truth",
                            "Zero-filled",
                        ...
                        / "frequency_aware_unet_with_dc_example.png",
                    )
                    example_saved = True

    rows = [
        average_rows(zero_filled_rows, "zero_filled"),
        average_rows(fa_unet_rows, "fa_complex_unet"),
        average_rows(fa_unet_dc_rows, "fa_complex_unet_dc"),
    ]

    metrics_path = metrics_dir / "frequency_aware_unet_with_dc_metrics.csv"
    save_metrics_csv(rows, metrics_path)

    print("Frequency-Aware Complex U-Net + DC evaluation completed.")
    ...
    print(
        f"Saved example to: "
        f"{figures_dir / 'frequency_aware_unet_with_dc_example.png'}"
    )
