...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train KAN-gated Frequency-Aware Complex U-Net."
    )
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_loss < best_val_loss:
            ...
            checkpoint_path = checkpoints_dir / "kan_frequency_aware_unet_best.pt"
            torch.save(
                {
    ...
    metrics_path = metrics_dir / "kan_frequency_aware_unet_training_metrics.csv"
    save_training_metrics(history, metrics_path)

    loss_curve_path = figures_dir / "kan_frequency_aware_unet_loss_curve.png"
    plot_loss_curve(history, loss_curve_path)

    print("KAN-gated Frequency-Aware Complex U-Net training completed.")
    print(f"Best validation loss: {best_val_loss:.6f}")
    print(
        f"Saved checkpoint to: {checkpoints_dir / 'kan_frequency_aware_unet_best.pt'}"
    )
