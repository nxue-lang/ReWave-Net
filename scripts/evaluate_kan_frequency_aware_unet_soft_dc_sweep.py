...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate KAN-gated FA-ComplexUNet with soft k-space DC sweep."
    )
    ...
    parser.add_argument(
        "--checkpoint-path",
        type=str,
        default="outputs/checkpoints/kan_frequency_aware_unet_best.pt",
    )
...
def main() -> None:
    ...
    for strength in dc_strengths:
        avg = average_rows(
            ...
            method="kan_fa_complex_unet_soft_dc",
        )
        avg["dc_strength"] = strength
        summary_rows.append(avg)

    metrics_path = metrics_dir / "kan_frequency_aware_unet_soft_dc_sweep_metrics.csv"
    save_metrics_csv(summary_rows, metrics_path)

    print("KAN-gated FA-ComplexUNet soft DC sweep completed.")
    ...
    if example_target is not None and example_zero_filled is not None:
        ...
        save_image_grid(
            ...
            / "kan_frequency_aware_unet_soft_dc_sweep_example.png",
        )

    print(f"Saved metrics to: {metrics_path}")
    print(
        "Saved example to: "
        f"{figures_dir / 'kan_frequency_aware_unet_soft_dc_sweep_example.png'}"
    )
