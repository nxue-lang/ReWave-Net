...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate frequency-aware complex reconstruction with DC."
    )
    ...
    parser.add_argument(
        "--checkpoint-path",
        type=str,
        default="outputs/checkpoints/frequency_aware_recon_best.pt",
    )
...
def main() -> None:
    ...
    model = FrequencyAwareComplexRecon(
    ...
    zero_filled_rows = []
    frequency_aware_rows = []
    frequency_aware_dc_rows = []
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            for (
                ...
                frequency_aware_mag, _ = target_scaled_magnitude_pair(
                ...
                frequency_aware_dc_mag, _ = complex_image_target_scaled_magnitude_pair(
                ...
                frequency_aware_rows.append(
                    compute_complex_channel_metrics(
                        prediction_channels=prediction_np,
                        target_channels=target_complex_np,
                    )
                )
                frequency_aware_dc_rows.append(
                    compute_complex_image_metrics(
                        prediction_complex=dc_complex,
                        target_complex=target_complex,
                    )
                ...
                if not example_saved:
                    save_image_grid(
                        images=[
                            ...
                            zero_filled_mag,
                            frequency_aware_mag,
                            frequency_aware_dc_mag,
                            np.abs(frequency_aware_dc_mag - target_mag),
                        ],
                        titles=[
                            "Ground Truth",
                            "Zero-filled",
                        ...
                        output_path=figures_dir / "frequency_aware_with_dc_example.png",
                    )
                    example_saved = True

    rows = [
        average_rows(zero_filled_rows, "zero_filled"),
        average_rows(frequency_aware_rows, "frequency_aware"),
        average_rows(frequency_aware_dc_rows, "frequency_aware_dc"),
    ]

    metrics_path = metrics_dir / "frequency_aware_with_dc_metrics.csv"
    save_metrics_csv(rows, metrics_path)

    print("Frequency-Aware + Data Consistency evaluation completed.")
    ...
    print(
        f"Saved example to: " f"{figures_dir / 'frequency_aware_with_dc_example.png'}"
    )
